(function() {
    ...
    app.controller('GuitarCoursesBySchoolTermController', [ '$http' , function($http){
        ...
        $http.get('../assets/json/courseWare/2017/fall/current/musicCourses.json').then(function (response) {
            catalogController.guitarProgramCatalog = response.data;
        }, function (badResult) {
            console.log(badResult)
        });
        catalogController.guitarProgramCatalogBySection = [];
        $http.get('../assets/json/courseWare/2017/fall/current/musicCoursesBySection.json').then(function (response) {
